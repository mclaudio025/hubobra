...
const workflowPayload = {
  name: "HubObra - Atendimento Inteligente IA (Lia + Zé da Obra) Multimodal Uazapi + Chatwoot CRM + Realtime Supabase",
  settings: {
    executionOrder: "v1"
  },
  nodes: [
    ...
    {
      parameters: {
        jsCode: `// ⚙️ NORMALIZADOR DE EVENTOS DA UAZAPI
...
return items.map(item => ({
  json: {
    ...
    // NÓ NOVO: BUSCA DE ESTOQUE EM TEMPO REAL NO SUPABASE
    {
      parameters: {
        method: "GET",
      ...
      id: "fetch-supabase-realtime-catalog",
      name: "📦 Buscar Estoque em Tempo Real (Supabase)",
      type: "n8n-nodes-base.httpRequest",
      typeVersion: 4.3,
      position: [150, 300]
    },
    // NÓ NOVO: FORMATAR PRODUTOS EM TEMPO REAL E INJETAR NO CONTEXTO
    {
      parameters: {
        jsCode: `// 🔄 FORMATAR CATÁLOGO EM TEMPO REAL PARA O AGENTE IA
const items = $input.all();
const initialData = $('⚙️ Normalizar Mensagem').first().json;
...
Responda ao cliente com atenção, gentileza e precisão baseando-se RIGOROSAMENTE nos produtos e fotos acima:`,
        options: {
...
for (const item of items) {
  ...
  let clientText = $('⚙️ Normalizar Mensagem').first().json.messageText;
  try {
    if ($('🔄 Montar Catálogo Dinâmico').first()?.json?.messageText) {
      clientText = $('🔄 Montar Catálogo Dinâmico').first().json.messageText;
    }
  } catch(_) {}
...
return results;`
    ...
    },
    {
      parameters: {
        ...
        sendBody: true,
        specifyBody: "json",
        jsonBody: "={{ { inbox_id: " + CHATWOOT_INBOX_ID + ", name: $json.name, phone_number: '+' + $json.phone } }}",
        options: {
          response: {
            response: {
              neverError: true
    ...
    },
    {
      parameters: {
        ...
        sendBody: true,
        specifyBody: "json",
        jsonBody: "={{ { source_id: $json.phone, inbox_id: " + CHATWOOT_INBOX_ID + ", contact_id: $json.payload?.contact?.id || $json.id, status: 'open', assignee_id: 1 } }}",
        options: {
          response: {
            response: {
              neverError: true
    ...
    },
    {
      parameters: {
        method: "POST",
        url: "=https://atendimento-chatwoot.q6zw3x.easypanel.host/api/v1/accounts/1/conversations/{{ $json.id || $json.conversation?.id }}/messages",
        sendHeaders: true,
        headerParameters: {
          parameters: [
            { name: "api_access_token", value: CHATWOOT_TOKEN },
        ...
        sendBody: true,
        specifyBody: "json",
        jsonBody: "={{ { content: $('📝 Formatar Resposta WhatsApp').first().json.clientText || 'Mensagem do cliente', message_type: 'incoming' } }}",
        options: {
          response: {
            response: {
              neverError: true
    ...
    },
    {
      parameters: {
        method: "POST",
        url: "=https://atendimento-chatwoot.q6zw3x.easypanel.host/api/v1/accounts/1/conversations/{{ $('💬 Chatwoot: Abrir Conversa').first().json.id || $('💬 Chatwoot: Abrir Conversa').first().json.conversation?.id }}/messages",
        sendHeaders: true,
        headerParameters: {
          parameters: [
            { name: "api_access_token", value: CHATWOOT_TOKEN },
  ...
  connections: {
    ...
    "É Imagem / Foto?": {
      main: [
        ...
        [
          {
            node: "📦 Buscar Estoque em Tempo Real (Supabase)",
            type: "main",
            index: 0
          }
        ]
    ...
    "📦 Buscar Estoque em Tempo Real (Supabase)": {
      main: [
        [
          {
            node: "🔄 Montar Catálogo Dinâmico",
            type: "main",
            index: 0
          }
        ]
...
async function deploy() {
  console.log('🚀 Atualizando workflow n8n com BUSCA DE ESTOQUE EM TEMPO REAL NO SUPABASE...');
  const cleanPayload = {
  ...
  const res = await requestN8N('/workflows/' + WORKFLOW_ID, 'PUT', cleanPayload);
  console.log('Status do PUT:', res.status);
  if (res.status === 200) {
    console.log('✅ WORKFLOW N8N COM ESTOQUE EM TEMPO REAL ATUALIZADO COM SUCESSO!');
    const act = await requestN8N('/workflows/' + WORKFLOW_ID + '/activate', 'POST');
    console.log('Status de Ativação:', act.status);
    console.log('\n🎉 SUCESSO TOTAL! Agora, a cada mensagem recebida, o n8n consulta o Supabase ao vivo.');
    console.log('Qualquer produto que você cadastrar no site ou no app scanner estará disponível para a Lia e o Zé da Obra instantaneamente!');
  } else {
